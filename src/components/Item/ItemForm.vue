<template>
  <div class="container">
    ...
    <p class="text-muted mt--1">{{ locInfo.address }}</p>
      ...
      <div class="input-group mt-3">
        <div class="form-floating">
          ...
          >            
            ...
            <option v-for="(cat, i) in categoriesList" :key="i" :value="cat.id">
              {{ cat.name }}
            </option>
          </select>
          <label for="catId" style="left: 37%">Click To Select Category Name</label>
        </div>
      </div>
      <div class="error-message" v-if="v$.catId.$error">
